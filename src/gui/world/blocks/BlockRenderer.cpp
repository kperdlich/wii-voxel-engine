...
void BlockRenderer::Rebuild()
{
	for(auto it = m_positions->begin(); it != m_positions->end(); ++it)
	{
		...
		BlockFaceVisibiltyData blockRenderData = *(*it)->pFaceData;
		...
		if ( blockRenderData.bBottomFace )
		{
			GX_Begin(GX_QUADS, GX_VTXFMT0, 4);
				// bottom side
				GX_Position3f32(vertices[6].x, vertices[6].y, vertices[6].z);
...
void BlockRenderer::DrawFocusOnSelectedCube(Vector3f& blockWorldPosition)
{
	// see http://www.matrix44.net/cms/wp-content/uploads/2011/03/ogl_coord_object_space_cube.png
	guVector vertices[8] = {
